...
Каждая семантика использует триаду: **точка** (`dot`), **фон бейджа** (`bg`), **текст бейджа** (`text`).

| Семантика | Назначение | Точка | Фон | Текст |
| --- | --- | --- | --- | --- |
| Success | Активно, оплачено, завершено | `bg-emerald-500` | `bg-emerald-50` | `text-emerald-700` |
| Warning | Приглашён, ожидает, приостановлено | `bg-amber-500` | `bg-amber-50` | `text-amber-700` |
| Danger | Срочно, просрочено | `bg-rose-500` | `bg-rose-50` | `text-rose-700` |
| Info (primary) | В работе, основная метрика | `bg-indigo-500` | `bg-indigo-50` | `text-indigo-700` |
| Process | В обработке | `bg-sky-500` | `bg-sky-50` | `text-sky-700` |
| Analysis | Анализ документов | `bg-violet-500` | `bg-violet-50` | `text-violet-700` |
| Neutral | Заблокирован, остановлено | `bg-gray-400` | `bg-gray-100` | `text-gray-600` |

**Критичное правило:** не использовать голый красный/зелёный — только `rose-*` и `emerald-*`. Это мягкий, менее агрессивный оттенок для B2B-интерфейса.
...
Универсальный паттерн: точка + фон + текст.
